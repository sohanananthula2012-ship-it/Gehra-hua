...
const sideMemText = document.getElementById('sideMemText');
const transInner  = document.getElementById('transInner');
...
// ════════════════════════════════════════════
//  SIDE MEMORIES (left column, cursive)
// ════════════════════════════════════════════
const sideMemories = [
  "wo pehli call — 3 ghante, 3 minute jaisi.",
  "tera hasna... sabse pyaara sound hai.",
  "tu door hai, par har gaane mein hai.",
  "kitna lucky hoon main, sochta reh jaata hoon.",
  "tera naam aate hi sab thoda better lagta hai.",
  "ye gaana sunte hue, tu yaad aati hai — hamesha.",
];

let lastSideMem = -1, sideTimer = null;

function showSideMem() {
  let idx;
  do { idx = Math.floor(Math.random() * sideMemories.length); }
  while (idx === lastSideMem && sideMemories.length > 1);
  lastSideMem = idx;

  sideMemText.classList.remove('show');
  setTimeout(() => {
    sideMemText.textContent = sideMemories[idx];
    requestAnimationFrame(() => {
      requestAnimationFrame(() => sideMemText.classList.add('show'));
    });
  }, 600);

  sideTimer = setTimeout(() => {
    sideMemText.classList.remove('show');
    setTimeout(() => { if (!mainAudio.paused) showSideMem(); }, 1200);
  }, 7500);
}

// ════════════════════════════════════════════
//  BUILD LYRICS + TRANSLATIONS DOM
// ════════════════════════════════════════════
let lyricsData = [];
...
function buildTranslations() {
  transInner.innerHTML = '';
  translations.forEach((t, i) => {
    const p = document.createElement('p');
    p.className = 'trans-line';
    p.dataset.idx = i;
    p.textContent = t;
    transInner.appendChild(p);
  });
}

// ── 3-line lyrics display (prev/active/next only) ──
let curLineIdx = -1;
...
function updateLyrics(time) {
  ...
  const prev = idx > 0 ? lyricsData[idx - 1].text : '';
  const curr = idx >= 0 ? lyricsData[idx].text : '';
  const next = idx >= 0 && idx < lyricsData.length - 1 ? lyricsData[idx + 1].text : '';

  lyricsInner.innerHTML = `
    <p class="lyric prev">${prev}</p>
    <p class="lyric active">${curr}</p>
    <p class="lyric next">${next}</p>
  `;
  ...
  // Scroll active translation into center of right col
  const activeTrans = transInner.querySelector('.trans-line.active');
  if (activeTrans) activeTrans.scrollIntoView({ block: 'center', behavior: 'smooth' });
}
